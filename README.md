...
==🔵Sidecar mode== (iPad as extended display via macOS Sidecar): all 5 keys collapse to ==🟣"toggle to other screen"== — direction is ignored because the iPad's spatial position is fluid. Detected automatically by the screen named `Sidecar Display (AirPlay)`. In sidecar mode, ==🟢dual mode== (laptop + one external monitor), and ==🟢single-screen mode==, [L010 shove](openfolder:///Users/sara/Library/CloudStorage/Dropbox/projects/log/2025/hammerspoon/stepper/features/L010-move-to-resize) applies per-screen — ==🟢fn+arrow shoves into edges instead of jumping to the neighbor display==. ==🟣Pure mechanical absorb==: pushing past an edge squeezes the visible frame ([move-to-resize.lua](openfile:///Users/sara/Library/CloudStorage/Dropbox/projects/log/2025/hammerspoon/stepper/lua/move-to-resize.lua)); subsequent move-back is a normal slide, not a stretch-back.
